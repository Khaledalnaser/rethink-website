...
function saveHistory(history, newTopic) {
  try {
    ...
    console.error('saveHistory failed:', e.message);
  }
}
...
async function generateArticle(history) {
  ...
  const used = history.slice(-20).map(h => h.topic).join('، ');
  ...
  try {
    ...
    if (!obj.title || !obj.desc || !obj.body) throw new Error('Generated article missing required fields');
...
async function generateNews() {
  ...
  try {
    ...
    const obj = JSON.parse(raw.replace(/```json|```/g, '').trim());
    if (!obj.title || !obj.desc) throw new Error('Generated news missing required fields');
...
async function generateBusiness() {
  ...
  try {
    ...
    const obj = JSON.parse(raw.replace(/```json|```/g, '').trim());
    if (!obj.title || !obj.desc) throw new Error('Generated business news missing required fields');
    return { tag, title: obj.title, desc: obj.desc, date: arabicDate() };
  } catch (e) {
...
async function main() {
  ...
  try {
    ...
    throw new Error('Failed to read data.json: ' + e.message);
  }
